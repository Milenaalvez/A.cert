...
const FORM_URLS = [
  'https://www.trt10.jus.br/certidao_online/jsf/publico/certidaoOnline.jsf',
  'https://www.trt10.jus.br/certidao_online/',
...
export class TRTConnector implements IConnector {
  ...
  async consultar(
    ...
    try {
      LOG(`Navegando...`);
      // Passo 0: estabelece sessao no dominio antes de ir pro JSF
      try {
        await page.goto('https://www.trt10.jus.br/', { waitUntil: 'domcontentloaded', timeout: 15000 });
        await wait(1500);
      } catch {}

      let loaded = false;
      for (const url of [FORM_URLS[0], ...FORM_URLS]) {
        try {
          // Movimento de mouse antes de navegar (menos robotic)
          ...
          if (textLen > 100) { loaded = true; break; }
        } catch (e: any) {
          LOG(`URL falhou: ${e.message.slice(0, 80)}`);
          await wait(2000);
        }
      }
      ...
      if (!loaded) {
        await page.close();
        return { status: 'error', orgao: this.nome, dataConsulta, error: 'Nao foi possivel acessar o TRT10 (403)' };
      }
      LOG(`URL ok: ${page.url()}`);
