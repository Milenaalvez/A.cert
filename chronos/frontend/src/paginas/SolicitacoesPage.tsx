...
export function SolicitacoesPage({ user }: { user?: { id: string; role: string; name: string; avatar?: string | null; permissions?: string[] } | null }) {
  ...
  const [viewMode, setViewMode] = useState<"resolver" | "all">("resolver")
  ...
  }, [tickets, filterStatus, search, isManager, user?.id, viewMode])
  ...
  async function handleAssign() {
    ...
    setAssigning(true)
    try {
      const updated = await apiTickets.assign(selectedId)
      setDetailTicket(updated)
      setTickets(prev => prev.map(t => t.id === selectedId ? updated : t))
    } catch (err: any) {
      setError(err.message)
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <PageHeader
        ...
        subtitle={viewMode === "resolver" ? "Solicitações atribuídas a você" : "Todas as solicitações"}
        actions={
          <button
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {detailTicket.assignedTo !== user?.id && detailTicket.status !== "ENCERRADO" && (
                      <button
                        ...
                        disabled={assigning}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] hover:bg-[var(--accent-primary)]/20 transition-colors"
                      >
                        {assigning ? <Loader2 size={12} className="animate-spin" /> : <LifeBuoy size={12} />}
