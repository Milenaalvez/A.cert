...
export async function list(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const tickets = await ticketService.listTickets(req.user!.userId, req.user!.role, req.user!.companyId, assignedToMe)
    res.json(tickets)
  } catch (err) { next(err) }
}
...
export async function assign(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const ticketId = req.params['id'] as string
    const result = await ticketService.assignTicket(ticketId, req.user!.userId, req.user!.companyId)
    if (!result) {
      res.status(404).json({ error: 'Solicitação não encontrada' })
      return
    }
